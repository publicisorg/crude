...
const styles2 = "border relative hover:scale-105 flex flex-row gap-4 cursor-pointer justify-start items-center hover:brightness-150 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-xl px-5 py-2.5 text-center duration-300 shadow-lg truncate w-32 justify-center";
...
export function ItemFolder(props: any) {
    ...
    return (
        <>
            <div onClick={handleClick} className={styles} key={props.index} style={{ borderColor: props.borderColor, backgroundColor: props.cardBg }}>
...
export function ItemImage(props: any) {
    ...
    return (
        <>
            <div onClick={handleClick} className={`${styles} aspect-square justify-center`} key={props.index} style={{ borderColor: props.borderColor, backgroundColor: props.cardBg }}>
    ...
    return (
        <>
            ...
            {show && <div onClick={handleClick} className="z-50 w-full h-screen fixed bg-black/50 flex justify-center items-center inset-0 flex-col gap-4 duration-300">
                ...
                <span onClick={() => openEditor(true)} className={`${styles2} aspect-auto`} style={{ borderColor: props.borderColor, backgroundColor: props.cardBg }}>Editar</span>
                <span onClick={handleClick} className="bg-red-500 rounded-full top-4 right-4 cursor-pointer w-10 h-10 flex justify-center items-center text-3xl absolute drop-shadow-xl duration-200 hover:text-red-500 hover:bg-white">X</span>
            </div>}
...
export function ItemNotSupported(props: any) {
    ...
    return (
        <>
            <a href={'https://desarrollodesitios0.site/2023/newsletters/' + props.path.join('/') + '/' + props.label} download={'https://desarrollodesitios0.site/2023/newsletters/' + props.path.join('/') + '/' + props.label} className={styles} key={props.index} style={{ borderColor: props.borderColor, backgroundColor: props.cardBg }}>
