  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (error) {
        ...
        setError(""); // Limpiar el mensaje de error si el inicio de sesión fue exitoso
        navigate("/");
      }
    } catch (error) {
      console.error(error);
        ...
        <form onSubmit={handleSubmit} className="flex text-left items-center flex-col gap-2 w-full">
          <div className="flex flex-col justify-center items-center w-full">
            ...
            <div className="m-auto flex justify-center items-center">
              <button className="border border-white/10 hover:brightness-150 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center duration-300 mt-10 mb-4 bg-gray-600">
                ENTRAR
              </button>
            </div>
          </div>
